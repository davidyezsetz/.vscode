...
class GitRevisionCodeLensProvider {
    ...
    provideCodeLenses(document, token) {
        return __awaiter(this, void 0, void 0, function* () {
            const data = gitProvider_1.GitProvider.fromGitContentUri(document.uri);
            const gitUri = new gitProvider_1.GitUri(vscode_1.Uri.file(data.fileName), data);
            const lenses = [];
            const log = yield this.git.getLogForFile(gitUri.fsPath, gitUri.sha, gitUri.repoPath, undefined, 2);
            if (!log)
                return lenses;
